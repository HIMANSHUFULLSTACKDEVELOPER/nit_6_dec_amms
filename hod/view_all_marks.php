...
$years_query = "SELECT DISTINCT academic_year FROM paper_marks pm
...
$stmt->bind_param("i", $department_id);
$stmt->execute();
$academic_years = $stmt->get_result();
...
$sections_query = "SELECT DISTINCT section FROM classes WHERE department_id = ? AND section IS NOT NULL ORDER BY section";
...
if ($selected_section && $selected_academic_year && $selected_exam_type) {
    // Build query to get all marks for the filtered section in this department
    $query = "SELECT pm.*, 
    ...
    $params = [$department_id, $selected_section, $selected_academic_year, $selected_exam_type];
    $types = "issi";
...
<head>
    ...
    <title>Section-wise Marks Report - HOD</title>
    ...
    <style>
        * {
        ...
        @media print {
            ...
            .btn, .filter-section, .dept-badge { display: none; }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Detailed student performance analysis - HOD Dashboard</p>
            <div class="dept-badge">
                🏢 <?php echo htmlspecialchars($department['dept_name']); ?> Department
            </div>
        </div>
        ...
        <div class="report-card">
            <div class="empty-state">
                ...
                <h3>No Data Found</h3>
                <p>No marks found for the selected filters in your department. Please check if marks have been uploaded for this section.</p>
            </div>
        </div>
        <?php endif; ?>
    </div>
    ...
    function exportToExcel() {
        ...
        a.href = url;
        a.download = 'section_marks_report_<?php echo htmlspecialchars($department['dept_name']); ?>_' + new Date().toISOString().slice(0,10) + '.csv';
        a.click();
