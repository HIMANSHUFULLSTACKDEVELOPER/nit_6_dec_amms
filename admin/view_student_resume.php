...
if ($student_id == 0) {
    header("Location: student_resumes.php");
    exit();
}
...
if (!$data) {
    echo "<script>alert('Student not found!'); window.location.href='student_resumes.php';</script>";
    exit();
}

if (!$data['objective']) {
    echo "<script>alert('This student has not created a resume yet!'); window.location.href='student_resumes.php';</script>";
    exit();
}

$theme = isset($data['theme']) ? $data['theme'] : 'professional';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Resume - <?php echo htmlspecialchars($data['full_name']); ?></title>
    ...
    <style>
        * {
        ...
        .theme-modern .resume-header .contact-info {
            display: flex;
            gap: 20px;
            flex-wrap: wrap;
            margin-top: 20px;
        ...
        .theme-creative .resume-header .contact-info {
            display: flex;
            gap: 20px;
            flex-wrap: wrap;
            margin-top: 20px;
        ...
        .theme-minimal .resume-header .contact-info {
            display: flex;
            gap: 20px;
            flex-wrap: wrap;
            margin-top: 20px;
...
<body>
    <div class="controls">
        ...
        <a href="student_resumes.php" class="btn btn-secondary">← Back to Resumes</a>
    </div>

    <div class="resume theme-<?php echo htmlspecialchars($theme); ?>">
        <!-- Header -->
        <div class="resume-header">
            <h1><?php echo htmlspecialchars($data['full_name']); ?></h1>
            <div style="font-size: 18px; opacity: 0.9; margin: 10px 0;">
                <?php echo htmlspecialchars($data['class_name'] . ' - ' . $data['section']); ?> | <?php echo htmlspecialchars($data['dept_name']); ?>
            </div>
            <div class="contact-info">
                <span>📧 <?php echo htmlspecialchars($data['email']); ?></span>
                <span>📱 <?php echo htmlspecialchars($data['phone']); ?></span>
                <span>🎓 Roll: <?php echo htmlspecialchars($data['roll_number']); ?></span>
            </div>
        </div>
